...
typealias STACK_FUN = (List<Int>) -> List<Int>
...
object Forth {
    fun evaluate(vararg line: String): List<Int> {
        ...
        line
            ...
            .forEach { list ->
                if (list.first() == ":") {
                    val list2 = list.drop(1).dropLast(1)
                    ...
                    operators.put(
                        word,
                        userDefine(list2.drop(1).map { mapFun(operators, it) })
                    )
                } else {
                    list.forEach {
    ...
    fun userDefine(funs: List<STACK_FUN>): STACK_FUN = funs.reduce { acc, fn -> { fn(acc(it)) } }
}
